...
class TcpConn
{
  ...
  public:
    TcpConn(int linkNumm,char * localIpp,short localPortt,char *remoteIpp,short remotePortt,bool ipBindd,bool portBindd,
        bool nonBlockk,bool naglee,bool reuseAddrr);
    ~TcpConn();
    virtual int run();//对于客户端为connect 对于服务器为listen accecpt(两者的返回值有点不一样)
